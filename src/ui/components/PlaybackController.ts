...
import type { TitleDisplay } from './TitleDisplay.js';

/**
 * Interface for waveform scrubber component
 * Used for type-safe interaction with WaveformScrubber
 */
export interface WaveformScrubberInterface {
    updateSeekBar?(): void;
    // Additional methods may be added by WaveformScrubber class
    // Using intersection type would be better but requires importing the class
}

export class PlaybackController {
    audioAnalyzer: AudioAnalyzer;
    waveformScrubber: WaveformScrubberInterface | null;
    ...
    isSeeking: boolean;
    seekUpdateInterval: ReturnType<typeof setInterval> | null;
    isLoopEnabled: boolean;
    isRandomEnabled: boolean;
    onTrackChange: (() => void) | null;
    onPlayRandomTrack: (() => void) | null;
    onLoadTrack: (() => Promise<void>) | null;
    onLoadFirstTrack: (() => Promise<void>) | null;
    onLoadNextTrack: (() => Promise<void>) | null;
    onLoadPreviousTrack: (() => Promise<void>) | null;
    
    constructor(audioAnalyzer: AudioAnalyzer, waveformScrubber: WaveformScrubberInterface | null = null, titleDisplay: TitleDisplay | null = null) {
        ...
        this.onLoadFirstTrack = null;
        this.onLoadNextTrack = null;
        this.onLoadPreviousTrack = null;
    }
    ...
    async handlePlayControl(): Promise<void> {
        ...
        try {
            ...
            ShaderLogger.error('Error controlling playback:', error);
        }
    }
    ...
    updatePlayControlButton(): void {
        ...
        if (!this.audioAnalyzer.audioElement) {
            ...
            const playIcon = this.playControlBtn.querySelector('.play-icon') as HTMLElement | null;
            const pauseIcon = this.playControlBtn.querySelector('.pause-icon') as HTMLElement | null;
    ...
    setupAudioElementListeners(): void {
        if (this.audioAnalyzer.audioElement) {
            ...
            this.audioAnalyzer.audioElement.addEventListener('timeupdate', () => {
                ...
                if (this.titleDisplay && audioElement && audioElement.duration && isFinite(audioElement.duration)) {
                    const playbackProgress = audioElement.currentTime / audioElement.duration;
                    this.titleDisplay.checkTitleDisplay(playbackProgress, audioElement.duration);
                }
            });
    ...
    updatePlaybackModeButtonState(): void {
        ...
        const randomIcon = this.playbackModeBtn.querySelector('.random-icon') as HTMLElement | null;
        const loopIcon = this.playbackModeBtn.querySelector('.loop-icon') as HTMLElement | null;
     ...
     * @param seconds - Seconds to skip backward
     */
     ...
     */
    updateSeekBar(): void {
        if (this.waveformScrubber && typeof this.waveformScrubber.updateSeekBar === 'function') {
