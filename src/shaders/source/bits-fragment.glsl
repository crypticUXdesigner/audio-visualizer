...
void main() {
    ...
    for(float i = 0.0; i < 100.0; i++) {
        ...
        p.z -= t;
        // GLSL ES 1.0: Calculate j from iteration count (j = 0.4 * 2^iter)
        for(float iter = 0.0; iter < 10.0; iter++) {
            float j = 0.4 * pow(2.0, iter);
            if (j >= 30.0) break;
            ...
            p += cos(pRounded - z * 0.1).yzx / j;
        }
        d = length(sin(p.xy)) * 0.1;
        z += d;
        o += (cos(z + vec4(6.0, 1.0, 2.0, 3.0)) + 1.0) / d;
    }
