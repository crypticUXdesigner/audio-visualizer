...
export class TimeOffsetManager {
    timeOffset: number;
    smoothedTimeOffset: number;
    baseTimeOffsetAccumulationRate: number;
    baseTimeOffsetDecayRate: number;
    maxTimeOffset: number;
    timeOffsetAccumulateThreshold: number;
    timeOffsetDecayThreshold: number;
    timeOffsetCubicBezier: CubicBezierConfig;
    loudnessControls: {
        loudnessAnimationEnabled?: boolean;
        loudnessThreshold?: number;
    } | null;
    
    constructor(config: TimeOffsetConfig = {}) {
     ...
     */
    setLoudnessControls(controls: { loudnessAnimationEnabled?: boolean; loudnessThreshold?: number } | null): void {
     ...
     * @param volume - Trigger signal strength (0-1), clamped internally
     * @returns Easing factor (0-1) that multiplies accumulation rate
     */
    getTimeOffsetEasingFactor(volume: number): number {
     ...
     * @param audioData - Audio data with volume and estimatedBPM (can be null)
     * @param deltaTime - Time since last update in seconds
     * @example
     * // Called every frame in render loop
     * timeOffsetManager.update(audioData, 0.016); // ~60fps
     */
    update(audioData: ExtendedAudioData | null, deltaTime: number): void {
     ...
     * @param audioData - Audio data with estimatedBPM (can be null)
     * @param deltaTime - Time since last update in seconds
     */
     ...
     * @returns Smoothed time offset value
     */
    getSmoothedOffset(): number {
