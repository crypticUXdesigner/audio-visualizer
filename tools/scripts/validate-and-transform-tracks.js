// Script to transform old track IDs and validate them against the Audiotool API
// This will rebuild the track registry with only valid tracks

import { getTrack } from './src/core/AudiotoolTrackService.js';
...
async function validateAndBuildRegistry() {
      ...
      batch.map(async (oldId) => {
        ...
        try {
          ...
          if (result.success && result.track) {
            ...
            const username = track.contributor_names?.[0] || 'audiotool';
            const songName = track.display_name;
          ...
          console.log(`❌ ${oldId} → ${newId} (error: ${error.message})`);
        }
  ...
  validTracks.sort((a, b) => a.songName.localeCompare(b.songName));
...
`;
  ...
  if (invalidTracks.length > 0) {
    ...
    for (const track of invalidTracks) {
      console.log(`   ${track.oldId} → ${track.newId} (${track.reason})`);
    }
  }
